...
namespace EXON.GradedEssay.Control
{
     public partial class ucWrittingPhucTra : UserControl
     {
          ...
          private string MaDe(int contestantShiftID)
          {
               CONTESTANTS_TESTS ct = _ContestantTestService.GetByContestantShiftId(contestantShiftID);
               if (ct != null)
               {
                    return ct.TestID.ToString();
               }
               return null;
               ...
               if (CurrentShiftID != -1 && CurrentRoomTestID != -1 && CurrentSubjectID != -1)
               {
                    ...
                    var listContestant = (from cs in _ContestantShiftService.GetAllByDivisionShiftID(CurrentDs.DivisionShiftID)
                                          ...
                                          select new
                                          {
                                               ...
                                               TestID = MaDe(cs.ContestantShiftID),
          ...
          private void gvMain_CellContentClick(object sender, DataGridViewCellEventArgs e)
          {
               ...
               try
               {
                    ...
                    string ContestanCode = gvMain.CurrentRow.Cells["cMTS"].Value.ToString();
...

                    {
                         try
                         {
                              ...
                              frm.Show();
                              frm.UpdateValue2(1);
                              GetAnswerForAll(id, TestNumberIndex, CurrentDs.DivisionShiftID, ContestanCode);
                              frm.UpdateValue2(2);
          ...
          public int GetAnswerForAll(int id, string TestNumberIndex, int DivisionShiftID, string contestantCode)
          {
               MTAQuizDbContext db = new MTAQuizDbContext();
               ...
               if (listans.Count != 0)
               {
                    ...
                    //                    ChoosenAnwser = GetIDAnswer(listans, sqt.SubQuestionID)
                    //               }
                    //     ).OrderBy(x => x.NO).ToList();
                    int number = 0;
                    lstQuestion = (from ctt in db.CONTESTANTS_TESTS.Where(x => x.ContestantShiftID == id).ToList()
                                   from tdt in db.TEST_DETAILS.Where(x => x.TestID == ctt.TestID).OrderBy(x => x.NumberIndex).ToList()
                                   from qt in db.QUESTIONS.Where(x => x.QuestionID == tdt.QuestionID).ToList()
                                   from sqt in db.SUBQUESTIONS.Where(x => x.QuestionID == qt.QuestionID).ToList()
                                   select new Question
                                   {
                                        NO = ++number,
                                        ...
                                        SubQuestionID = sqt.SubQuestionID,
                                        SubQuestionContent = sqt.SubQuestionContent,
                                        NumberOfSubquestion = qt.NumberSubQuestion,
                                        NumberOfIndex = tdt.NumberIndex,
                                        Testid = ctt.TestID,
                                        Answer = getans(listans, sqt.SubQuestionID),
                                        Score = (float)sqt.Score,
                                        Type = sqt.QUESTION.Type ?? default(int),
                    ...
                    if (lstQuestion.Count > 0)
                    {
                         ...
                         {
                              return SaveFileAnswer(lstQuestion, TestNumberIndex, ct.ContestName, FolderNameOutput, FileNameOutput, contestantCode);
          ...
          private int GetIDAnswer(List<ListAnwser> listans, int subQuestionID)
          {
               _AnswerService = new AnswerService();
          ...
          //
          private int GetIDAwserIsCorrect(int subQuestionID)
          {
               _AnswerService = new AnswerService();
               ANSWER ans = new ANSWER();
          ...
          private int SaveFileAnswer(List<Question> list, string numberIndex, string ContestName, string FolderNameOutput, string FileNameOutput, string contestantCode)
          {
               try
               {
                    CreateFileHead(numberIndex, ContestName, FolderNameOutput, list[0].Testid, contestantCode);
                    foreach (Question QA in list)
                    {
                         ...
                         {
                              string FileNameQA = FolderNameOutput + QA.QuestionID + ".rtf";
                              ...
                              rtfNumberIndex.Append(QA.QuestionContent +"\n", TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                              MTAQuizDbContext db = new MTAQuizDbContext();
                              List<SUBQUESTION> lst_SubQuestion = new List<SUBQUESTION>();
                              lst_SubQuestion = db.SUBQUESTIONS.Where(x => x.QuestionID == QA.QuestionID).ToList();
                              foreach (SUBQUESTION SQA in lst_SubQuestion)
                              {
                                   SubQuestion sqa = new SubQuestion();
                                   sqa.SubQuestionID = SQA.SubQuestionID;
                                   sqa.SubQuestionContent = SQA.SubQuestionContent;
                                   sqa.Score = (float)SQA.Score;
                                   sqa.No = QA.NO - QA.NumberOfSubquestion + 1;
                                   QA.NO += 1;
                                   TXTextControl.TextControl rtfSQA = new TXTextControl.TextControl();
                                   this.Controls.Add(rtfSQA);
                                   rtfSQA.Text = string.Format("Câu hỏi {0} ({1} điểm): \n ", sqa.No, sqa.Score);
                                   rtfSQA.Font = new System.Drawing.Font("Times New Roman", 13, FontStyle.Bold);
                                   rtfSQA.Select(rtfSQA.Text.Length, 0);
                                   rtfSQA.Append(SQA.SubQuestionContent, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                                   rtfNumberIndex.Select(rtfNumberIndex.Text.Length, 0);
                                   string s;
                                   rtfSQA.Save(out s, TXTextControl.StringStreamType.RichTextFormat);
                                   ...
                                   //IsCorrect 
                                   string Iscorrect1 = "";
                                   List<int> lstAnwserID = _AnswerService.getAllBySubQuestionID(SQA.SubQuestionID).Select(x => x.AnswerID).ToList();
                                   List<ANSWER> lstAnwser = _AnswerService.getAllBySubQuestionID(SQA.SubQuestionID).ToList();
                                   foreach (ANSWER anw in lstAnwser)
                                   {
                                        if (anw.IsCorrect)
                                        {
                                             ...
                                             {
                                                  case 0: Iscorrect1 = "A"; break;
                                                  case 1: Iscorrect1 = "B"; break;
                                                  case 2: Iscorrect1 = "C"; break;
                                                  case 3: Iscorrect1 = "D"; break;
                                             }
                                        }
                                   }
                                   if (QA.ChoosenAnwser != -1)
                                   {
                                        int IndexOfAnwser = lstAnwserID.IndexOf(QA.ChoosenAnwser);
                                        switch (IndexOfAnwser)
                                        {
                                             case 0: ABCD1 = "A"; break;
                                             case 1: ABCD1 = "B"; break;
                                             case 2: ABCD1 = "C"; break;
                                             case 3: ABCD1 = "D"; break;
                                        }
                                   }
                                   if (QA.AnswerForRegular != "")
                                   {
                                        rtfa.Text = "Đáp án đúng: " + Iscorrect1 + "\n" + "Chọn đáp án: " + ABCD1 + "   ";
                                        rtfa.Select(rtfa.Text.Length, 0);
                                        rtfa.Append(QA.AnswerForRegular, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                                   ...
                                   rtfNumberIndex.Append(s1, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                              }
                         ...
                         else
                         {
                              ...
                              //IsCorrect 
                              string Iscorrect = "";
                              List<int> lstAnwserID = _AnswerService.getAllBySubQuestionID(QA.SubQuestionID).Select(x => x.AnswerID).ToList();
                              List<ANSWER> lstAnwser = _AnswerService.getAllBySubQuestionID(QA.SubQuestionID).ToList();
                              foreach (ANSWER anw in lstAnwser)
                              {
                                   if (anw.IsCorrect)
                                   {
                                        ...
                                        switch (IndexOfIsCorrect)
                                        {
                                             case 0: Iscorrect = "A"; break;
                                             case 1: Iscorrect = "B"; break;
                                             case 2: Iscorrect = "C"; break;
                                             case 3: Iscorrect = "D"; break;
                                        }
                                   }
                              }
                              if (QA.ChoosenAnwser != -1)
                              ...
                              if (QA.AnswerForRegular != "")
                              {
                                   rtfa.Text = "Đáp án đúng: " + Iscorrect + "\n" + "Chọn đáp án: " + ABCD + "   ";
                                   rtfa.Select(rtfa.Text.Length, 0);
                                   rtfa.Append(QA.AnswerForRegular, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                                   rtfa.Font = new System.Drawing.Font("Times New Roman", 13, FontStyle.Regular);
                              }
          ...
          private void CreateFileHead(string numberIndex, string ContestName, string FolderNameOutput, int TestID, string contestantCode)
          {
               ...
               this.FindAndReplace(objApp, "Số phách", "SBD");
               this.FindAndReplace(objApp, "NumberIndex", contestantCode);
