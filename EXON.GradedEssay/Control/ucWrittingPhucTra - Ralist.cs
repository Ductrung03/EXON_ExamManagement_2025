          ...
          public int GetAnswerForAll(int id, string TestNumberIndex, int DivisionShiftID, string contestantCode)
          {
               ...
               if (listans.Count != 0)
               {
                    ...
                    lstQuestion = (from ctt in db.CONTESTANTS_TESTS.Where(x => x.ContestantShiftID == id).ToList()
                         ...
                         ).OrderBy(x => x.NumberOfIndex).ToList();
               ...
               try
               {
                    ...
                    {                 
                         ...
                         if (QA.QuestionContent != null)
                         {
                              ...
                              rtfNumberIndex.SelectedRtf = QA.QuestionContent;
                              ...
                              foreach (SUBQUESTION SQA in lst_SubQuestion)
                              {
                                   ...
                                   QA.NO += 1;
                                   RichTextBox rtfSQA = new RichTextBox();
                                   float score = (float)Math.Round(sqa.Score, 2);
                                   rtfSQA.Text = string.Format("Câu hỏi {0} ({1} điểm): ", sqa.No, score);
                                   rtfSQA.AppendText("\n");
                                   rtfSQA.Font = new System.Drawing.Font("Times New Roman", 13, FontStyle.Bold);
                                   rtfSQA.Select(rtfSQA.TextLength, 0);
                                   rtfSQA.SelectedRtf = sqa.SubQuestionContent;
                                   rtfNumberIndex.Select(rtfNumberIndex.TextLength, 0);
                                   rtfNumberIndex.SelectedRtf = rtfSQA.Rtf;
