...
namespace EXON.GradedEssay.Control
{
     public partial class ucWrittingPhucTra : UserControl
     {
               ...
               try
               {
...

                    {
                         try
                         {
                              ...
                              GetAnswerForAll(id, TestNumberIndex, CurrentDs.DivisionShiftID, ContestanCode,testID);
                              frm.UpdateValue2(2);
          ...
          public int GetAnswerForAll(int id, string TestNumberIndex, int DivisionShiftID, string contestantCode,int testID)
          {
               MTAQuizDbContext db = new MTAQuizDbContext();
               ...
               if (listans.Count != 0)
               {
                    ...
                    if (lst_ques2print.Count > 0)
                    {
                         ...
                         {
                              return SaveFileAnswer(lst_ques2print, TestNumberIndex, ct.ContestName, FolderNameOutput, FileNameOutput, contestantCode,testID);
          ...
          private int SaveFileAnswer(List<EXON.SubModel.Question2Print> list, string numberIndex, string ContestName, string FolderNameOutput, string FileNameOutput, string contestantCode,int testID)
          {
               try
               {
                    CreateFileHead(numberIndex, ContestName, FolderNameOutput, testID, contestantCode);
                    foreach (EXON.SubModel.Question2Print ques2print in list)
                    {
                         ...
                         foreach (EXON.SubModel.SubQuestion2Print subques2print in ques2print.lst_subquestions)
                         {
                              ...
                              if (ques2print.Type != (int)QuizTypeEnum.Essay && ques2print.Type != (int)QuizTypeEnum.Fill && ques2print.Type != (int)QuizTypeEnum.FillAudio && ques2print.Type != (int)QuizTypeEnum.ReWritting)
                              {
                                   ...
                                   foreach (EXON.SubModel.Answer2Print ans2print in subques2print.lst_answers)
                                   {
                                        ...
                                        ANSWER ans = new ANSWER();
                                        ans.AnswerID = ans2print.AnswerID;
                                        ans.AnswerContent = ans2print.AnswerContent;
                                        ans.IsCorrect = ans2print.IsCorrect;
                                        lstAnwser.Add(ans);                                 
                                   }
                                   ...
                                   foreach (ANSWER anw in lstAnwser)
                                   {
                                        if(anw.IsCorrect)
                                        {
                                             ...
                                             {
                                                  case 0: firstLetter = "A"; break;
                                                  case 1: firstLetter = "B"; break;
                                                  case 2: firstLetter = "C"; break;
                                                  case 3: firstLetter = "D"; break;
                                             }
                                   ...
                                   string ABCD1 = "";
                                   //IsCorrect 
                                   if (GetIDAnswer(listans, subques2print.SubQuestionID) != -1)
                                   {
                                        int IndexOfAnwser = lstAnwserID.IndexOf(GetIDAnswer(listans, subques2print.SubQuestionID));
                                   ...
                                   if (getAnswerForRegulargetans(listans, subques2print.SubQuestionID) != "")
                                   {
                                        rtfAnswer.Text = "Đáp án đúng: " + firstLetter + "   ";
                                        rtfAnswer.Select(rtfAnswer.Text.Length, 0);
                                        string ans;
                                        rtfA.Save(out ans, TXTextControl.StringStreamType.RichTextFormat);
                                        rtfAnswer.Append(ans + "\n", TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);    
                                        rtfAnswer1.Text = "Chọn đáp án: " + ABCD1 + "   ";
                                        rtfAnswer1.Select(rtfAnswer1.Text.Length, 0);
                                        rtfAnswer1.Append(getAnswerForRegulargetans(listans, subques2print.SubQuestionID), TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                                        rtfAnswer.Select(rtfAnswer.Text.Length, 0);
                                        string ans1;
                                        rtfAnswer1.Save(out ans1, TXTextControl.StringStreamType.RichTextFormat);
                                        rtfAnswer.Append(ans1, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);

                                   }
                                   else
                                   {
                                        if (getans(listans, subques2print.SubQuestionID) != "")
                                        {
                                             rtfAnswer.Text = "Trả lời: \n";
                                             rtfAnswer.Select(rtfAnswer.Text.Length, 0);
                                             rtfAnswer.Append(getans(listans, subques2print.SubQuestionID), TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                                             rtfAnswer.Font = new System.Drawing.Font("Times New Roman", 13, FontStyle.Regular);
                                        }
                                        else
                                        {
                                             rtfAnswer.Text = "Trả lời: Thí sinh bỏ qua câu hỏi này";
                                             rtfAnswer.Font = new System.Drawing.Font("Times New Roman", 13, FontStyle.Bold);
                                        }
                              ...
                              else if (ques2print.Type == (int)QuizTypeEnum.Essay)
