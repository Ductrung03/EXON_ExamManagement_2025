...
namespace EXON.MONITOR
{
    public partial class frmMain : MetroFramework.Forms.MetroForm
    {
        ...
        private void rbtnSpeaking1_Click(object sender, EventArgs e)
        {
            ...
            // Tính lại điểm cho các thí sinh chưa tính điểm tổng (ANSWERSHEET.TestScore = null)
            ReCalculate_Score();
        ...
        private void ReCalculate_Score()
        {
            try
            {
                ContestantTestService _ContesttantTestService = new ContestantTestService();
                AnswersheetDetailService _AnswersheetDetailService = new AnswersheetDetailService();
                AnswerService _AnswerService = new AnswerService();

                MTAQuizDbContext db = new MTAQuizDbContext();
                List<ANSWERSHEET> lst_aws = db.ANSWERSHEETS.Where(p => p.TestScores == null).ToList();
                foreach (ANSWERSHEET aws in lst_aws)
                {
                    List<ANSWERSHEET_DETAILS> lstaws_dt = _AnswersheetDetailService.getAllByAnswerID(aws.AnswerSheetID).ToList();
                    ANSWER aw = new ANSWER();
                    int ansID;
                    float SumSCore = 0;
                    foreach (ANSWERSHEET_DETAILS item in lstaws_dt)
                    {
                    ...
                    aws.TestScores = SumSCore;
                }
                db.SaveChanges();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Có lỗi trong quá trình tính lại điểm tổng cho thí sinh\n\n" + ex.Message, "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }           
        }

        private void rbtnDeleteTest_Click(object sender, EventArgs e)
        {
            SplashScreenManager.ShowSplashScreen();
            foreach (Form f in this.MdiChildren)
            {
                if (f.GetType() != typeof(frmDeleteTest))
                {
                    f.Close();
                    f.Dispose();
            ...
            frmDeleteTest form1 = new frmDeleteTest(_ContestID, _LocationID);
            form1.MdiParent = this;
            form1.Show();
            SplashScreenManager.CloseForm();
        }
        ...
        private void ribbonButton4_Click(object sender, EventArgs e)
        {
            ...
            foreach (Form f in this.MdiChildren)
            {
                if (f.GetType() != typeof(frmPrintTest))
                {
                    f.Close();
                    f.Dispose();
            ...
            frmPrintTest form1 = new frmPrintTest(_ContestID, _LocationID);
            form1.MdiParent = this;
            form1.Show();
        ...
        private void ribbonButton6_Click(object sender, EventArgs e)
        {
               ...
               foreach (Form f in this.MdiChildren)
               {
                    if (f.GetType() != typeof(frmReportPart))
                    {
                         f.Close();
                         f.Dispose();
               ...
               frmReportPart form1 = new frmReportPart(_ContestID, _LocationID);
               form1.MdiParent = this;
               form1.Show();
          ...
          private void ribbonButton8_Click(object sender, EventArgs e)
          {
               ...
               foreach (Form f in this.MdiChildren)
               {
                    if (f.GetType() != typeof(frmPrintBagOfTest))
                    {
                         f.Close();
                         f.Dispose();
               ...
               frmPrintBagOfTest form1 = new frmPrintBagOfTest(_ContestID, _LocationID);
               form1.MdiParent = this;
               form1.Show();
        ...
        private void rbpChangePass_Click(object sender, EventArgs e)
        {
            ExaminationcouncilStaffService _ExaminationcouncilStaffService = new ExaminationcouncilStaffService();
            EXAMINATIONCOUNCIL_STAFFS exs = _ExaminationcouncilStaffService.GetByStaffID(EXON.MONITOR.Common.AppSession.StaffID);

            FrmDoiMatKhau frm = new FrmDoiMatKhau(exs);
            frm.ShowDialog();
            //db.SaveChanges();
        }
          ...
          private void ribbonButton10_Click(object sender, EventArgs e)
          {
               ...
               foreach (Form f in this.MdiChildren)
               {
                    if (f.GetType() != typeof(frmUpdateScore))
                    {
                         f.Close();
                         f.Dispose();
               ...
               frmUpdateScore form1 = new frmUpdateScore(_ContestID, _LocationID);
               form1.MdiParent = this;
               form1.Show();
          ...
          private void ribbonButton11_Click(object sender, EventArgs e)
          {
               ...
               foreach (Form f in this.MdiChildren)
               {
                    if (f.GetType() != typeof(frmThongKeDapAn))
                    {
                         f.Close();
                         f.Dispose();
               ...
               frmThongKeDapAn form1 = new frmThongKeDapAn(_ContestID, _LocationID);
               form1.MdiParent = this;
               form1.Show();
          ...
          {
               frmUpdateQuestion form1 = new frmUpdateQuestion();
