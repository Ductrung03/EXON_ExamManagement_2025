...
namespace EXON.GradedEssay.Report
{
    public partial class frmRpKetQuaTongNNDauRa : Form
    {
        ...
        List<int> lst_subquesionsID_audio = new List<int>();
        List<ANSWER> lst_correct_answer = new List<ANSWER>();
        ...
        public frmRpKetQuaTongNNDauRa(int LocationID, string unit, string subjectname)
        {
            InitializeComponent();
            InitializeService();
        ...
        private float KetQuaNghe(int contestantShiftID)
        {
            ...
            if (ct != null)
            {
                ...
                if (anw != null)
                {
                    int ansID;
                    _AnswersheetDetailService = new AnswersheetDetailService();
                    List<int?> lstaws = _AnswersheetDetailService.getAllByAnswerID(anw.AnswerSheetID).Where(p => lst_subquesionsID_audio.Contains(p.SubQuestionID)).Select(p => p.ChoosenAnswer).ToList();
                    List<double?> lst_score = lst_correct_answer.Where(p => lstaws.Contains(p.AnswerID)).Select(p => p.SUBQUESTION.Score).ToList();
                    SumSCore = (float)lst_score.Sum();
                    return SumSCore;
        ...
        private void FrmRpKetQuaTongNNDauRa_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                lst_correct_answer = db.ANSWERS.Where(p => p.IsCorrect == true).ToList();
                lst_subquesionsID_audio = db.SUBQUESTIONS.Where(p => p.QUESTION.Audio != null).Select(p => p.SubQuestionID).ToList();
                ...
                if (_Unit != string.Empty)
                {
                    ...
                    foreach (CONTESTANT ct in lstct)
                    {
                        ...
                        kq.STT = -1;
                        lstcs = _ContestantShiftService.GetAllByContestantID(ct.ContestantID).ToList().Where(p => p.SCHEDULE.SUBJECT.SubjectName == _SubjectName).ToList(); ;
                    ...
                    //    _ContestantService = new ContestantService();
                    //    List<CONTESTANT> lstct = new List<CONTESTANT>();
                    //    lstct = _ContestantService.GetAll().Where(x => x.Unit == _Unit).ToList();
                    //    List<KetQuaTongNNDauRa> lstKQ = new List<KetQuaTongNNDauRa>();
                    //    int stt = 1;
                    //    foreach (CONTESTANT ct in lstct)
                    //    {
                    ...
                    //this.rpvSum.LocalReport.Render()
                }
            }
            catch (Exception ex)
            {
