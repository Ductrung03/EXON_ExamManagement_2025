    ...
    public partial class ucWritting : UserControl
    {
        ...
        private int CurrentRoomTestID
        {
            get
            {
                ...
                {
                    int result = -1;
                    if (cbRoomTest.SelectedValue != null)
                    {
                        result = int.Parse(cbRoomTest.SelectedValue.ToString());
        ...
        private void cbRoomTest_SelectedValueChanged(object sender, EventArgs e)
        {
            if (CurrentShiftID != -1 && CurrentRoomTestID != -1)
            {
                ...
                txtDivisionShiftID.Text = CurrentDs.DivisionShiftID.ToString();
        ...
        private void cbSubject_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            if (CurrentShiftID != -1 && CurrentRoomTestID != -1 && CurrentSubjectID != -1)
            {
                ...
                //                           ContestantCode = cs.ContestantShiftID,

                //                      }).ToList();
                //List<ContestantResult> listContestantr = new List<ContestantResult>();
                //foreach (var item in listContestant)
                //{
                //     ContestantResult a = (
                //        from tn in _TestNumberService.GetByContestanstshiftID(item.ContestantCode)
                ...
                //             Score2 = GetScore2(tn.ContestantShiftID ?? 0),
                //             DaHoanThanh = CheckHasAnswer(item.ContestantCode) ? "Đã hoàn thành" : "Không làm bài thi viết",

                //             PrintAnswer = "Xem"
                //        }).FirstOrDefault();
                //     listContestantr.Add(a);
                //}
                ...
                join cs in _ContestantShiftService.GetAllByDivisionShiftID(CurrentDs.DivisionShiftID) on tn.ContestantShiftID equals cs.ContestantShiftID
                where cs.SCHEDULE.SubjectID == CurrentSubjectID && cs.DIVISION_SHIFTS.Status == (int)Common.StatusDivisionShift.STATUS_COMPLETE
                select new ContestantResult
                {
                    TestNumberIndex = CurrentDs.DivisionShiftID.ToString() + "." + tn.TestNumberIndex.ToString(),
                    ContestantShiftID = tn.ContestantShiftID ?? 0,
        ...
        private string GetScore2(int contestantShiftID)
        {
            if (contestantShiftID > 0)
            {
                CONTESTANTS_TESTS ct = _ContestantTestService.GetByContestantShiftId(contestantShiftID);
                if (ct != null)
                {
                    ...
                    if (anw != null)
                    {
                        return anw.EssayPoints.ToString();
                    }
                    else
                    {
                        return string.Empty;
                    ...
                    if (gvMain.DataSource != null)
                    {
                        ...
                        foreach (DataGridViewRow row in gvMain.Rows)
                        {
                            ...
                            try
                            {
                                ...
                                string v2 = row.Cells["cScore2"].Value.ToString();
                                float score;
                                float score2;
                                // score = int.Parse(v);
                                CONTESTANTS_TESTS ct = _ContestantTestService.GetByContestantShiftId(contestantShiftID);
                                if (ct != null)
                                {
                                    ...
                                    {
                                        if (v2 != "")
                                            if (v2.IndexOf('.') >= 0) score2 = float.Parse(v2);
                                            else score2 = int.Parse(v2);
                                        else continue;
                                        anw.EssayPoints = score2;
        ...
        private void gvMain_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                MessageBox.Show(ex.Message + "erro code in line 567! ");
            }
        }
        ...
        private void OpenFileAnsewer(string testNumberIndex)
        {
            ...
            if (!Directory.Exists(Path.Combine(pathFileTemp.ToString(), CurrentDs.DivisionShiftID.ToString() + "." + SubjectName.Trim())))
            {
                MessageBox.Show("Chưa tạo bài làm");
                return;
            }
            object szPath = pathFileTemp + "\\" + dsID.ToString() + "." + SubjectName.Trim() + "\\" + testNumberIndex + ".docx";
        ...
        private void CreateDocument(List<Question> list, string numberIndex, string ContestName)
        {
            ...
            {
                MessageBox.Show(ex.Message + "erro code in line 956!");
        ...
        private void btnPrintResult_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(ex.Message + " erro code in line 1018!");
                MessageBox.Show("Không có dữ liệu!");
        ...
        private void btnCreateAll_Click(object sender, EventArgs e)
        {
                    ...
                    if (sum > 0)
                    {
                        ...
                        foreach (ContestantResult ct in listContestant)
                        {
                            if (ct.DaHoanThanh.Equals("Đã hoàn thành"))
                            {
                                ...
                                s((int)Per);
                                int Return = GetAnswerForAll(ct.ContestantShiftID, ct.TestNumberIndex, PathFolderTemp, CurrentDs.DivisionShiftID);
                                if (Return == -1)
                                {
                                    s(100);
                                    Loading.Close();
                                    Loading.Dispose();
                                    killProcessMSWord();
                                    MetroFramework.MetroMessageBox.Show(this, String.Format("Đã dừng quá trình tạo bài thi số phách {0}! Vui lòng chọn tạo lại.", ct.TestNumberIndex), "Lỗi!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                }
                                if (count == sum && Return == 1)
                                {
                                    ...
                                    Loading.Dispose();
                                    killProcessMSWord();
                                    MetroFramework.MetroMessageBox.Show(this, String.Format("Đã tạo {0} trong tổng số {1} bài làm xong!", count, sum), "Thông báo!", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                }
            ...
            {
                MessageBox.Show(ex.Message + " erro code in line 1126! ");
        ...
        public int GetAnswerForAll(int id, string TestNumberIndex, string PathFolderTemp, int DivisionShiftID)
        {
            MTAQuizDbContext db = new MTAQuizDbContext();
            listans = new List<ListAnwser>();
            listans = (from ctt in db.CONTESTANTS_TESTS.Where(x => x.ContestantShiftID == id)
            ...
            if (listans.Count != 0)
            {
                ...
                lstQuestion = (from ctt in db.CONTESTANTS_TESTS.Where(x => x.ContestantShiftID == id).ToList()
                               ...
                               && (x.Type == (int)QuizTypeEnum.Essay || x.Type == (int)QuizTypeEnum.ReWritting)).ToList()
                               from sqt in db.SUBQUESTIONS.Where(x => x.QuestionID == qt.QuestionID).ToList()
                                   // KHẮC PHỤC IN THIẾU ĐỀ CÂU HỎI LỚN KHI CÓ TỪ 2 CÂU HỎI LỚN TRỞ LÊN TRONG PHẦN VIẾT
                                   // chỗ này sắp xếp theo QuestionID và SubquestionID để lúc in report ra không bị thiếu đề của câu hỏi lớn
                                   // (trường hợp ko sắp xếp có thể bị thiếu đề của câu hỏi lớn nếu có từ 2 câu hỏi lớn trở lên)
                               orderby qt.QuestionID, sqt.SubQuestionID
                               select new Question
                               {
                                   NO = ++number,
                                   QuestionID = sqt.QuestionID,
                ...
                if (lstQuestion.Count > 0)
                {
                    ...
                    ct = _ContestService.GetById(_ContestID);
                    string SubjectName = _SubjectService.GetById(CurrentSubjectID).SubjectName;
                    string FileNameOutput = pathFileTemp.ToString() + "\\" + DivisionShiftID.ToString() + "." + SubjectName.Trim() + "\\" + TestNumberIndex + ".docx";//
                    string FolderNameOutput = pathFileTemp.ToString() + "\\" + DivisionShiftID.ToString() + "." + SubjectName.Trim() + "\\" + TestNumberIndex + "\\";

                    if (!Directory.Exists(Path.Combine(FolderNameOutput.ToString(), PathFolderTemp)))
        ...
        private int SaveFileAnswer(List<Question> list, string numberIndex, string ContestName, string FolderNameOutput, string FileNameOutput)
        {
            try
            {
                ...
                bool indexCauhoiLon = false;
                int previous_questionID = -1;
                foreach (Question QA in list)
                {
                    // KHẮC PHỤC IN THIẾU ĐỀ CÂU HỎI LỚN KHI CÓ TỪ 2 CÂU HỎI LỚN TRỞ LÊN TRONG PHẦN VIẾT
                    // list đã được sắp xếp theo QuestionID và SubquestionID
                    // mỗi câu hỏi con, ta xét câu hỏi lớn của nó có giống câu hỏi lớn của câu trước ko
                    // nếu giống thì ko in câu hỏi lớn ra file, ngược lại thì in câu hỏi lớn trước câu hỏi con
                    // (khắc phục tình trạng in thiếu đề của câu hỏi lớn khi có nhiều hơn 2 câu hỏi lớn trong 1 đề viết)
                    if (QA.QuestionID != previous_questionID)
                    {
                        previous_questionID = QA.QuestionID;
                        indexCauhoiLon = true;
                    }
                    if (QA.QuestionContent != null && indexCauhoiLon == true)
                    {
                        ...
                        //rtfNumberIndex.AppendText("\n");
                        //rtfNumberIndex.Font = new System.Drawing.Font("Times New Roman", 13, FontStyle.Bold);
                        TXTextControl.TextControl rtfNumberIndex = new TXTextControl.TextControl();
                        this.Controls.Add(rtfNumberIndex);
                        float score = (float)Math.Round(QA.Score, 2);
                        rtfNumberIndex.Text = string.Format("Câu hỏi {0} ({1} điểm): \n ", QA.NO, score);
                        rtfNumberIndex.Font = new System.Drawing.Font("Times New Roman", 13, FontStyle.Bold);
                        rtfNumberIndex.Select(rtfNumberIndex.Text.Length, 0);
                        rtfNumberIndex.Append(QA.SubQuestionContent, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                        TXTextControl.TextControl rtfQA = new TXTextControl.TextControl();
                        this.Controls.Add(rtfQA);
                        rtfQA.Select(rtfQA.Text.Length, 0);
                        rtfQA.Append(QA.QuestionContent, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                        rtfQA.Select(rtfQA.Text.Length, 0);
                        string s;
                        rtfNumberIndex.Save(out s, TXTextControl.StringStreamType.RichTextFormat);
                        rtfQA.Append(s, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                        ...
                        if (QA.Answer != "")
                        {
                            ...
                            rtfa.Append(QA.Answer, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                            rtfa.Font = new System.Drawing.Font("Times New Roman", 13, FontStyle.Regular);
                        }
                        else
                        {
                        ...
                        rtfQA.Select(rtfQA.Text.Length, 0);
                        string s1;
                        rtfa.Save(out s1, TXTextControl.StringStreamType.RichTextFormat);
                        rtfQA.Append(s1, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                        rtfQA.Save(FileNameQA, TXTextControl.StreamType.RichTextFormat);
                        indexCauhoiLon = false;
                    }
                    else
                    {
                        ...
                        rtfNumberIndex.Select(rtfNumberIndex.Text.Length, 0);
                        string s;
                        rtfa.Save(out s, TXTextControl.StringStreamType.RichTextFormat);
                        rtfNumberIndex.Append(s, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                        rtfNumberIndex.Save(FileNameQA, TXTextControl.StreamType.RichTextFormat);
                    }
                }
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show(ex.Message + " erro code in line 1306!");
                return -1;
            }
        ...
        private int Mergefile(string FileNameOutput, string FolderNameOutput, string numberIndex)
        {
            ...
            try
            {
                Word.Application wordApplication = new Word.Application { DisplayAlerts = WdAlertLevel.wdAlertsNone, Visible = false };
                ...
                foreach (Microsoft.Office.Interop.Word.Section wordSection in wordDocument.Sections)
                {
                    ...
                    footerRange.Fields.Add(footerRange, Type: Word.WdFieldType.wdFieldSectionPages);
                }
            ...
            {
                // Xử lý lỗi COMException
                MessageBox.Show("Có lỗi COMException xảy ra: line 1407! " + comEx.Message);
                return -1; // Hoặc giá trị thất bại khác
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message + " erro code in line 1412! ");
                return -1;
            }
        }
        private void CreateFileHead(string numberIndex, string ContestName, string FolderNameOutput, int TestID)
        {
            ...
            {
               MessageBox.Show(ex.Message + "   line 1446!");
            };
        }
        ...
        private void btnInDapAn_Click(object sender, EventArgs e)
        {
            try
            {
                if (CurrentSubjectID != -1)
                {
                    ...
                    List<PrintWritingAnswerQuestion> lstWriting = (
                                 from cts in _ContestantShiftService.GetAllByDivisionShiftID(ds.DivisionShiftID).Where(x => x.SCHEDULE.SubjectID == CurrentSubjectID).ToList()
                                 ...
                                 select new PrintWritingAnswerQuestion()
                                 {
        ...
        ///  In đáp án
        /// </summary>
        /// <param name="PrintWritingAnswerQuestio"></param>
                            ...
                            if (cursor < list.Count)
                            {
                                ...
                                if (list[cursor].QuestionContent != null)
                                {
                                    if ((cursor == 0) || (cursor > 0 && list[cursor].QuestionID != list[cursor - 1].QuestionID))
                                    {
                                        // câu hỏi lớn 
                                        RichTextBox rtfNumberIndex = new RichTextBox();
                                        rtfNumberIndex.Text = string.Format("Câu hỏi {0} ({1} điểm): ", 1, list[cursor].Score);
                                        rtfNumberIndex.AppendText("\n");
                                        rtfNumberIndex.Font = new System.Drawing.Font("Times New Roman", 13, FontStyle.Bold);
                                        rtfNumberIndex.Select(rtfNumberIndex.TextLength, 0);
                                        rtfNumberIndex.SelectedRtf = list[cursor].SubQuestionContent;
        ...
        private void btnInTatCaBaiLam_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                CurrentDs = _DivisionShiftService.GetByShiftAndRoomTest(CurrentShiftID, CurrentRoomTestID);
                string subjectName = _SubjectService.GetById(CurrentSubjectID).SubjectName;
                if (!Directory.Exists(Path.Combine(pathFileTemp.ToString(), CurrentDs.DivisionShiftID.ToString() + "." + subjectName.ToString())))
                {
                    MessageBox.Show("Chưa tạo bài làm");
                    return;
                }
                else
                {
                    ...
                    string outputFileName = (pathFileTemp.ToString() + "\\" + CurrentDs.DivisionShiftID.ToString() + "." + subjectName.ToString() + "\\" + CurrentDs.DivisionShiftID.ToString() + subjectName + ".docx");

                    File.Delete(outputFileName);
                    string[] filePaths = Directory.GetFiles(pathFileTemp.ToString() + "\\" + CurrentDs.DivisionShiftID.ToString() + "." + subjectName.ToString() + "\\");
            ...
            catch (Exception ex)
            {
                ...
                Log.Instance.WriteErrorLog(Properties.Resources.MSG_LOG_ERROR, "Loi" + ex.ToString());
            }
        ...
        private void BtnPhucTra_Click(object sender, EventArgs e)
        {
            try
            {
                if (CurrentSubjectID != -1)
                    ...
                    if (sum > 0)
                    {
                        ...
                        foreach (ContestantResult ct in listContestant)
                        {
                            if (ct.DaHoanThanh.Equals("Đã hoàn thành"))
                            {
                                ...
                                s((int)Per);
                                /// so phach
                                int Return = GetAnswerForAllPhucTra(ct.ContestantShiftID, ct.TestNumberIndex, PathFolderTemp);
                                if (Return == -1)
                                {
                                    s(100);
                                    Loading.Close();
                                    Loading.Dispose();
                                    killProcessMSWord();
                                    MetroFramework.MetroMessageBox.Show(this, "Đã dừng quá trình bài thi!", "Lỗi!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    break;
                                }
                                if (count == sum && Return == 1)
                                {
        ...
        public int GetAnswerForAllPhucTra(int id, string TestNumberIndex, string PathFolderTemp)
        {
            MTAQuizDbContext db = new MTAQuizDbContext();
            ...
            if (listans.Count != 0)
            {
                ...
                lstQuestion.RemoveAll(x => x.Type == (int)QuizTypeEnum.Speaking);
                ...
                if (lstQuestion.Count > 0)
                {
                    ...
                    return CreateDocumentForAllPhucTra(lstQuestion, TestNumberIndex, ct.ContestName, PathFolderTemp);
                }
        ...
        private int CreateDocumentForAllPhucTra(List<Question> list, string numberIndex, string ContestName, string PathFolderTemp)
        {
            killProcessMSWord();
            objApp = new Word.Application();
            object missing = Missing.Value;
...

                    {
                            ...
                            if (cursor < list.Count)
                            {
                                ...
                                if (list[cursor].QuestionContent != null)
                                {
                                    if ((cursor == 0) || (cursor > 0 && list[cursor].QuestionID != list[cursor - 1].QuestionID))
                                    {
                                        ...
                                        TXTextControl.TextControl rtfNumberIndex = new TXTextControl.TextControl();
                                        this.Controls.Add(rtfNumberIndex);
                                        rtfNumberIndex.Text = string.Format("Câu hỏi {0} ({1} điểm): \n", list[cursor].NO, list[cursor].Score);
                                        rtfNumberIndex.Font = new System.Drawing.Font("Times New Roman", 13, FontStyle.Bold);
                                        rtfNumberIndex.Select(rtfNumberIndex.Text.Length, 0);
                                        rtfNumberIndex.Append(list[cursor].SubQuestionContent, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);

                                        rtfQuestion.Text = list[cursor].QuestionContent;
                                        rtfQuestion.Select(rtfQuestion.Text.Length, 0);
                                        string s;
                                        rtfNumberIndex.Save(out s, TXTextControl.StringStreamType.RichTextFormat);
                                        rtfQuestion.Append(s, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                                        bool CheckCopy = true;
                                        while (CheckCopy)
                                        {
                                            Clipboard.Clear();
                                    ...
                                    else
                                    {
                                        ...
                                        rtfNumberIndex.Font = new System.Drawing.Font("Times New Roman", 13, FontStyle.Bold);
                                        rtfQuestion.Select(rtfQuestion.Text.Length, 0);
                                        rtfQuestion.Append(list[cursor].SubQuestionContent, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                                        string s;
                                        rtfNumberIndex.Save(out s, TXTextControl.StringStreamType.RichTextFormat);
                                        rtfQuestion.Append(s, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                                        Clipboard.Clear();
                        ...
                        else
                        {
                            if (cursor < list.Count)
                            {
                                //RichTextBox rtfa = new RichTextBox();
                                TXTextControl.TextControl rtfa = new TXTextControl.TextControl();
                                if (list[cursor].AnswerForRegular != "")
                                {
                                    rtfa.Text = "Trả lời: ";
                                    rtfa.Select(rtfa.Text.Length, 0);
                                    rtfa.Append(list[cursor].AnswerForRegular, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                                    rtfa.Font = new System.Drawing.Font("Times New Roman", 13, FontStyle.Regular);
                                }
                                else
                                {
                                    if (list[cursor].Answer.Length < 100 && list[cursor].Answer != "")
                                    {
                                        TXTextControl.TextControl rtfReply = new TXTextControl.TextControl();
                                        this.Controls.Add(rtfReply);
                                        rtfReply.Select(rtfReply.Text.Length, 0);
                                        rtfa.Append(list[cursor].Answer, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                                        string s;
                                        rtfa.Save(out s, TXTextControl.StringStreamType.RichTextFormat);
                                        ...
                                        rtfa.Font = new System.Drawing.Font("Times New Roman", 13, FontStyle.Regular);
                                        rtfa.Select(rtfa.Text.Length, 0);
                                        rtfa.Append(s, TXTextControl.StringStreamType.RichTextFormat, TXTextControl.AppendSettings.None);
                                    }
                                    else if (list[cursor].Answer != "")
                                    {
                                        rtfa.Text = "Trả lời: ";
        ...
        private void BtnConfirmTeacher_Click(object sender, EventArgs e)
        {
            try
            {
                DIVISION_SHIFTS CurrentDs = new DIVISION_SHIFTS();
                ...
                CurrentDs = _DivisionShiftService.GetByShiftAndRoomTest(CurrentShiftID, CurrentRoomTestID);
                frmReportPhieuDiemThiVietXacNhan frm = new frmReportPhieuDiemThiVietXacNhan(CurrentDs.DivisionShiftID, stff1.FullName, stff2.FullName, CurrentSubjectID);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        {
            //if (e.RowIndex == gvMain.CurrentRow.Index)
            //{
            //    int i = gvMain.CurrentRow.Index;
...
internal class Question
{
    ...
    public List<SubQuestion> lst_subquestions { get; set; }
    ...
    public string AnswerForRegular { get; set; }
    public int ChoosenAnwser { get; set; }
}
public class SubQuestion
{
    ...
    public float Score { get; set; }
    public int No { get; set; }
    public List<Answer> lst_answers { get; set; }
    public SubQuestion()
    {
        SubQuestionID = -1;
        SubQuestionContent = "";
